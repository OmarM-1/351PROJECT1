...
string recvFileName()
{
	...
	if(msgrcv(msqid, &holdMsg, sizeof(holdMsg.fileName), FILE_NAME_TRANSFER_TYPE, 0) == -1){
		perror("msgrcv");
		exit(1);
	}
	/* TODO: return the received file name */
	fileName = string(holdMsg.fileName);
        return fileName;
}
 /**
...
unsigned long mainLoop(const char* fileName)
{
	...
	{	
		...
		// switched from message to rcv
		if (msgrcv(msqid, &rcv, sizeof(rcv) - sizeof(rcv.mtype), SENDER_DATA_TYPE, 0) < 0){
			perror("msgrcv");
			fclose(fp);
			exit(1);
		}
		...
		if(msgSize != 0)
		{
			 ...
			 * of type ackMessage with mtype field set to RECV_DONE_TYPE. 
			 ...
			 if (msgsnd(msqid, &nextmsg, sizeof(nextmsg) - sizeof(nextmsg.mtype), 0) < 0){
				 perror("msgsnd");
				 //fclose(fp);
				 exit(1);
			 }
